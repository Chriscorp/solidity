...
contract FileObjectField_v1 is VersionField {
    struct Field {
        bool isCreated;
    ...
    function FileObjectField_v1(ContractNameService _cns) VersionField(_cns, "FileObject") public {}

    /** OVERRIDE */
    function setDefault(bytes32 _id) private {
        bytes32[3] memory emptyArray;
        fields[_id] = Field({ isCreated: true, hashes: emptyArray, readerId: 0, writerId: 0, tmpWriteTimestamp: 0, writeTimestamp: 0 });
    }
    ...
    function create(bytes32 _id, bytes32[3] _hashes, bytes32 _readerId, bytes32 _writerId, uint _tmpWriteTimestamp) public onlyByNextVersionOrVersionLogic {
        require(!exist(_id));
        fields[_id] = Field({ isCreated: true, hashes: _hashes, readerId: _readerId, writerId: _writerId, tmpWriteTimestamp: _tmpWriteTimestamp, writeTimestamp: 0 });
    }
